...
JSEND;
    ...
    private function writeCSS(): void
    {
        ...
        try {
            // Tema de usuario (theme_ID) vs preset (arelance-corporate)
            if (strpos($this->templateId, 'theme_') === 0) {
                $themeId = (int)substr($this->templateId, 6);
                logError("DEBUG writeCSS: user theme detected, themeId={$themeId}, templateId={$this->templateId}");
                require_once __DIR__ . '/Auth.php';
                $auth = new \ScormConverter\Auth();
                $db = $auth->getDb();
                $theme = $tm->getUserThemeById($db, $themeId);
                if ($theme) {
                    logError("DEBUG writeCSS: theme found — name={$theme['name']}, logo_filename=" . ($theme['logo_filename'] ?? 'NULL'));
                    $css = $tm->buildCSSFromColors($theme['color_primary'], $theme['color_accent']);
                    $copiedAssets = $tm->copyUserThemeLogo($theme['logo_filename'], $this->tempPath);
                    logError("DEBUG writeCSS: copiedAssets=" . json_encode($copiedAssets));
                    $this->templateData = [
                        'name' => $theme['name'],
                        'colors' => ['primary' => $theme['color_primary'], 'accent' => $theme['color_accent']],
                        '_copied_assets' => $copiedAssets
                    ];
                } else {
                    logError("DEBUG writeCSS: theme NOT found for id={$themeId}, falling back to arelance");
                    $css = $tm->buildCSS('arelance-corporate');
                }
            } else {
            ...
            logError("ERROR writeCSS: exception caught — " . $e->getMessage());
...
.corpo-logo-img{height:80px;width:auto;object-fit:contain;border-radius:6px}
...
.topnav-logo-img{height:44px;width:auto;object-fit:contain;margin-right:.25rem}
